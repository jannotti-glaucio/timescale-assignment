...
func TestSummarizeResults(t *testing.T) {
	...
	result1 := parsers.QueryResult{
		Duration: time.Duration(250),
		MinUsage: 20,
		MaxUsage: 80,
	}
	result2 := parsers.QueryResult{
		Duration: time.Duration(350),
		MinUsage: 30,
		MaxUsage: 70,
	}
	result3 := parsers.QueryResult{
		Duration: time.Duration(150),
		MinUsage: 10,
		MaxUsage: 90,
	}
	results := parsers.QueryResults{result1, result2, result3}
...
func TestGetMedian(t *testing.T) {
	t.Run("one result", func(t *testing.T) {
		...
		results := parsers.QueryResults{result}
	...
	t.Run("multipe even results", func(t *testing.T) {
		...
		result4 := parsers.QueryResult{
			Duration: time.Duration(400),
			MinUsage: 40,
			MaxUsage: 60,
		}
		results := parsers.QueryResults{result1, result2, result3, result4}
