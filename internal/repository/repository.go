...
func OpenConnection(ctx context.Context) (*pgx.Conn, error) {
...
func RunQuery(ctx context.Context, conn *pgx.Conn, hostname string, startDate time.Time, endDate time.Time) (float32, float32, error) {
	...
	if err != nil {
		exceptions.ThrowError("Error reading fields from query: %v", err)
	}
